...
export function generateKakaoMapDirectionsUrl(
  ...
  // 카카오내비 앱 URL만 반환 (kakaonavi:// 스키마 사용)
  return `kakaonavi://navigate?name=${encodeURIComponent(name)}&x=${lng}&y=${lat}&coordType=wgs84&vehicleType=1&rpOption=1&routeInfo=true`;
}
...
export function startKakaoNavi(
  ...
  try {
    // 카카오내비 앱 직접 연결 URL 생성 (kakaonavi:// 스키마 사용)
    const appUrl = `kakaonavi://navigate?name=${encodeURIComponent(locationData.name)}&x=${locationData.lng}&y=${locationData.lat}&coordType=wgs84&vehicleType=1&rpOption=1&routeInfo=true`;
...
export function shareKakaoNavi(locationData: LocationData): void {
  ...
  try {
    // 카카오내비 앱 직접 연결 URL 생성 (kakaonavi:// 스키마 사용)
    const appUrl = `kakaonavi://share?name=${encodeURIComponent(locationData.name)}&x=${locationData.lng}&y=${locationData.lat}&coordType=wgs84`;
