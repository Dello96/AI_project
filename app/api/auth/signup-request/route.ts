...
export async function POST(request: NextRequest) {
  try {
    ...
    let authUser: { id: string; email_confirmed_at?: string | null } | null =
      null;
    ...
    if (authError) {
      ...
      if (authError.code === 'over_email_send_rate_limit') {
        const { data: fallbackUserData, error: fallbackError } =
        ...
        if (fallbackError) {
          ...
          if (
            ...
            return NextResponse.json(
              {
                error:
                  '이미 가입된 이메일 주소입니다. 비밀번호 로그인을 시도해주세요.',
              },
              { status: 409 }
            );
          }
    ...
    if (profileError) {
      ...
      return NextResponse.json(
        {
          error:
            '회원 정보 저장 중 오류가 발생했습니다. 관리자에게 문의해주세요.',
        },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({
      ...
      message: usedAdminFallback
        ? '가입 요청이 접수되었습니다. 입력한 이메일의 메일함을 확인해주세요'
        : '가입이 완료되었습니다.',
